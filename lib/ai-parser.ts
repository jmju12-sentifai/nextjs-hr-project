...
function withIds(schema: any) {
  ...
  const repairSteps = (steps: any[] = []): any[] =>
    steps
      ...
      if (out.type === "formula") {
        ...
        if (!out.a) out.a = "오늘";
        if (out.mode === "diff" && !out.b) out.b = "오늘";
  ...
  const isUsableStep = (s: any): boolean => {
    ...
    if (s.type === "branch") {
      ...
      const sideHasValue = (typ: string, txt: any, toks: any[]) => {
        ...
        // text 모드 — 기본 placeholder("참"/"거짓") 만이면 무의미
        if (typeof txt !== "string") return false;
        const t = txt.trim();
        if (!t) return false;
        if (t === "참" || t === "거짓") return false;
        return true;
      };
    ...
    if (s.type === "date") {
      // a 가 없으면 무효
      return typeof s.a === "string" && s.a.trim() !== "";
    }
    if (s.type === "llm") {
  ...
  // numberPromote 결과를 nonNumericNames 에 반영 (산식에 쓰인 이름은 numeric 으로 간주, 단 명백한 텍스트 이름 제외)
  for (const n of numberPromote) {
    if (!looksLikeText(n)) nonNumericNames.delete(n);
  }
  // 필터링 (무효 step 제거)
  ...
  if (Array.isArray(schema.vars) && numberPromote.size > 0) {
    schema.vars = schema.vars.map((v: any) => {
      ...
      if (numberPromote.has(v.name) && v.type !== "number" && !looksLikeText(v.name)) {
  ...
  if (Array.isArray(schema.vars)) {
    const REG_HINTS = [
      ...
      /률$/, /율$/, /비율$/, /연령$/, /나이$/, /정년/, /기준일$/, /기준년수$/,
      /기간$/, /일수$/, /운영모델/, /정책/, /등급기준/, /적용연령/,
    ...
    schema.vars = schema.vars.map((v: any) => {
      ...
      const name = v.name;
      // 이미 명시된 grp 가 분명한 의미(규정/개인) 이면 그대로 둠 — 단, 분명히 규정 키워드면 보정
      const looksReg = REG_HINTS.some((re) => re.test(name));
      const looksPer = PER_HINTS.some((re) => re.test(name));
      if (looksReg && !looksPer && v.grp !== "규정") {
        return { ...v, grp: "규정" };
      }
      if (looksPer && !looksReg && v.grp !== "개인") {
        return { ...v, grp: "개인" };
      }
      // 둘 다 아니거나 둘 다 매칭이면 원래 값 유지 (개인 기본값)
      if (v.grp !== "규정" && v.grp !== "개인") return { ...v, grp: "개인" };
      return v;
    });
  }
  ...
  // ── 변수 필수(req) 자동 판정 — 분석 로직/리포트에서 실제로 쓰이는 변수만 req=true ──
  // 개인·규정 grp 무관하게 동일 규칙 적용. 이름 패턴 매칭은 사용 안 함.
  if (Array.isArray(schema.vars)) {
    const referenced = new Set<string>();
    const collectFromConds = (conds: any[] = []) => {
      for (const c of conds) {
        if (!c) continue;
        if (typeof c.a === "string" && c.aMode !== "val") referenced.add(c.a);
        if (typeof c.b === "string" && c.bMode !== "val") referenced.add(c.b);
      }
    };
    const collectFromTokens = (tokens: any[] = []) => {
      if (!Array.isArray(tokens)) return;
      for (const tok of tokens) {
        if (tok?.t === "var" && tok.name) referenced.add(tok.name);
      }
    };
    const collectFromSteps = (steps: any[] = []) => {
      for (const s of steps) {
        if (!s) continue;
        if (typeof s.a === "string") referenced.add(s.a);
        if (typeof s.b === "string") referenced.add(s.b);
        if (typeof s.ref === "string") referenced.add(s.ref);
        if (typeof s.min === "string") referenced.add(s.min);
        if (typeof s.max === "string") referenced.add(s.max);
        if (Array.isArray(s.items)) {
          for (const it of s.items) {
            if (typeof it === "string") referenced.add(it);
            else if (it?.ref) referenced.add(it.ref);
          }
        }
        // formula tokens
        collectFromTokens(s.tokens);
        // branch thenTok / elsTok
        collectFromTokens(s.thenTok);
        collectFromTokens(s.elsTok);
        // switch cases[].tokens 와 defaultTokens
        if (Array.isArray(s.cases)) {
          for (const c of s.cases) {
            collectFromTokens(c?.tokens);
          }
        }
        collectFromTokens(s.defaultTokens);
      }
    };
    const collectFromReport = (report: any[] = []) => {
      for (const e of report) {
        if (!e) continue;
        if (typeof e.bind === "string") referenced.add(e.bind);
        if (typeof e.bind2 === "string") referenced.add(e.bind2);
        if (Array.isArray(e.binds)) for (const b of e.binds) if (b) referenced.add(b);
    ...
    collectFromSteps(schema.shared?.steps);
    for (const p of schema.paths || []) {
      collectFromConds(p.conditions);
      collectFromSteps(p.steps);
      collectFromReport(p.report);
    }
    if (schema.fallback) {
      collectFromConds(schema.fallback.conditions);
      collectFromSteps(schema.fallback.steps);
      collectFromReport(schema.fallback.report);
    }
    ...
    schema.vars = schema.vars.map((v: any) => {
      if (!v || typeof v !== "object" || typeof v.name !== "string") return v;
      const referencedHit = referenced.has(v.name);
...
- **변수 이름 작성 규칙 (매우 중요)** — 공백/특수문자 없이.
  - **상위 > 하위 > 세부 분류** 순으로 작성. 단계 사이는 언더스코어(\`_\`) 로 구분.
  - ✅ 좋은 예 (경조사 도메인):
...
- **paths[].steps 설계 — 변수 이름 패턴 매칭이 아니라 "서비스 맥락 이해" 가 우선**
  ...
  ### Step 3. step 의 이름·역할을 비즈니스 용어로
  step.name 은 변수명 조합이 아니라 **그 step 이 비즈니스적으로 무엇을 하는지** 가 드러나야 한다.
  ❌ "본인구분지원금" (변수명 합치기) — 의미 모호
  ✅ "결혼축의금 산출" (대상자 관계에 따라 결혼 축의금을 결정) — 비즈니스 의미 명확
...
export async function generateAppSpecPreview(
  ...
  const parts: any[] = [{ text: SPEC_PREVIEW_PROMPT }];
  parts.push({ text: "\n\n===== [참고 문서] =====" });
  for (let i = 0; i < files.length; i++) {
    const sub = await fileToParts(files[i], `참고 문서 ${i + 1}`);
    parts.push(...sub);
...
export function previewToAppSchema(preview: AppSpecPreview): any {
  ...
  const tokenizeExpr = (expr: any): any[] => {
    ...
    while (i < s.length) {
      ...
      i = j;
      if (name) toks.push({ t: "var", name: resolveName(name) });
    }
    return toks;
  };
  ...
  const buildStep = (s: SpecPreviewStep): any => {
    ...
    switch (s.type) {
      ...
      case "formula": {
        // 수식 문자열(formula) 을 토큰으로 파싱 — 곱셈·괄호·step 참조 모두 보존.
        let tokens = tokenizeExpr(sa.formula);
...
function pathReportFromPreview(preview: AppSpecPreview, label: string): any[] {
  ...
  const personalCoreVars = (preview.vars || []).filter(
    (v) => v.grp === "개인" && v.category === "핵심"
  );
  const referencedInReport = new Set<string>();
  for (const e of out) {
    if (Array.isArray((e as any).binds)) for (const b of (e as any).binds) referencedInReport.add(b);
    if ((e as any).bind) referencedInReport.add((e as any).bind);
    if (e.kind === "note" && typeof (e as any).tpl === "string") {
      const tpl = (e as any).tpl as string;
      const matches = tpl.match(/\{([^}]+)\}/g) || [];
      for (const m of matches) referencedInReport.add(m.slice(1, -1).trim());
    }
  }
  // 메타성 변수는 자동 제외
  const META_EXCLUDE = /(비상연락처|연락처|이메일|email|메모|비고|주소|address|url|링크)/i;
  const unused = personalCoreVars
    .filter((v) => !referencedInReport.has(v.name))
    .filter((v) => !META_EXCLUDE.test(v.name));

  if (unused.length > 0) {
    // 개별 field 로 각각 추가 — fields(묶음)에 다 넣지 말 것.
    // 위치: 기존 fields 가 있으면 그 바로 뒤에, 없으면 note 앞에.
