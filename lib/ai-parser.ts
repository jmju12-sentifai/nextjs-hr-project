...
- 단, **같은 값을 같은 팔레트로 두 번 보여주는 중복은 금지** (예: 같은 산출을 card 두 개로). 한 값은 한 번만.
...
function pathReportFromPreview(
  ...
  if (valueSteps.length > 0) {
    ...
    const toAdd = valueSteps.filter((n) => !shown.has(n));
    // 결과물을 맨 앞으로 (강조)
    ...
    if (cards.length) {
      ...
      out.splice(noteAt >= 0 ? noteAt : out.length, 0, ...cards);
    }
  }
  ...
  // ── 같은 값 중복 제거 ──
  // 값을 표시하는 팔레트(card/field/calc/incexc/chart)가 같은 bind 를 두 번 보여주면 먼저 것만 유지.
  // (fields 묶음의 binds 도 "표시된 값" 으로 간주해, 같은 값을 개별 card/field 로 또 보여주지 않게 함.)
  ...
  for (const e of out) {
    if (e.kind === "fields" && Array.isArray((e as any).binds)) {
      deduped.push(e);
      for (const b of (e as any).binds) seenBind.add(b);
      continue;
    }
    if (VALUE_KINDS.has(e.kind) && (e as any).bind) {
      if (seenBind.has((e as any).bind)) continue; // 같은 값 중복 → 제거
      seenBind.add((e as any).bind);
    }
    deduped.push(e);
  }
  return deduped;
