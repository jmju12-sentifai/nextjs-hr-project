...

{
  ...
  "vars": [
    ...
    //     예시 칸이 "결혼 ↵ 값: 결혼/사망/출산" 형태면 → test="결혼", options=["결혼","사망","출산"] ("값:" 줄은 test 에 넣지 말 것).
  ...
  "_step_blocks": [
    ...
    //      "out": "sum" }                                      // sum|avg|max|min|count|pick|list
    //    tokens 의 var 는 컬럼명·전역 변수·"매핑값" 참조. map.value 는 숫자 또는 규정 변수 이름.
    //    out="list" 면 행별 결과 목록(리포트 list 카드로 노출), 그 외는 숫자 하나.
    //    out="pick" = 규정 표(rows) 조회 — 필터로 행을 특정(컬럼 vs 전역 이름)하고 tokens 에 꺼낼 컬럼:
    //    { "type":"rowcalc", "name":"직급하한액", "ref":"직급체계표", "out":"pick",
    //      "filters":[{"col":"최소경력","op":"<=","val":"최종경력연차"},{"col":"최대경력","op":">=","val":"최종경력연차"}],
    //      "tokens":[{"t":"var","name":"하한액"}] }   // 직급 수 제한 없음 — switch 펼치기 금지
  ...
  const repairSteps = (steps: any[] = []): any[] =>
    steps
      ...
      if (out.type === "formula") {
        ...
        if (!["list", "sum", "avg", "max", "min", "count", "pick"].includes(out.out)) out.out = "sum";
...
- **paths[].steps 설계 — 변수 이름 패턴 매칭이 아니라 "서비스 맥락 이해" 가 우선**
  ...
  - "규정 표(rows)에서 조건에 맞는 행의 값 하나 꺼내기" (직급 판정·직급별 밴드 조회) → rowcalc (out="pick",
    filters=[컬럼 vs 전역 산출 비교], expression=꺼낼 컬럼명). 표를 개별 변수·switch 로 펼치지 말 것.
  - "최저·최고 보정" → clamp
  ...
  - 양자택일 단순 분기 → branch
  - ⚠ **화면에 그대로 표시되는 branch** 의 참/거짓 출력은 사용자가 이해하는 도메인 라벨로.
    true/false·참/거짓·Y/N 같은 불리언을 그대로 출력하지 마라 (결과 화면에 "false" 가 떠서 무슨 뜻인지 알 수 없음).
    ✅ 출생반기 분기 → then="상반기", els="하반기"   (또는 then="당해 7월 1일", els="익년 1월 1일")
    ❌ then="true"/els="false", then="참"/els="거짓"  ← 사용자에게 의미 없음
    ⚠ 단, **뒤 산식이 더하거나 세는 중간 계산**이면 라벨이 아니라 숫자 1/0 으로 출력하라 (아래 "요건 충족 개수" 참고).
      이때 then="1", els="0" 이고 그 branch 는 리포트에 직접 노출하지 않는다.
  ...
  **산출 step (branch/switch 등) 으로 만들면 안 된다.** 그 판정은 **paths 의 진입조건 + fallback(미적용)** 구조가
  ...
  ### ✅ 예외 — 요건이 여러 개고 "몇 개를 충족했나" 를 세야 하는 경우
  ...
  ⚠ 요건별 branch 를 만들지 않으면 뒤의 합계 formula 가 없는 이름을 참조해
     그 이름들이 전부 "사용자 입력 변수" 로 잘못 생성된다 — 사용자가 판정 결과를 직접 입력하는 앱이 된다.
...
- 처음 출력 후 스스로 다음을 검토 — 보강·교정 뒤 최종 JSON:
  ...
  (9) **앱의 목적/판정 자체를 산출 step 으로 만든 게 있나?** (🚫 위 금지 규칙)
       ...
       - ✅ 단, 요건이 여러 개고 충족 개수로 결론이 갈리는 도메인이면 요건별 branch(then="1"/els="0") + 합계 formula + 최종 branch 는
         **반드시 있어야 한다** (위 "예외" 참고). 이 경우 요건별 branch 가 없으면 그게 누락이다.
  (9-b) **산식(formula)·조건이 참조하는 이름이 전부 변수 또는 앞선 산출로 정의돼 있나?**
       - 정의되지 않은 이름을 참조하면 그 이름이 "사용자 입력 변수" 로 자동 생성되어, 계산 결과를 사용자가 직접 입력하는 앱이 된다.
       - 특히 합계 formula 가 참조하는 요건별 충족 결과는 반드시 branch step 으로 먼저 정의하라.
...
**표 형태 정책**(직급체계표·페이밴드표·환산율표·수당표 등 행이 여러 개인 표)은 text 로 통째 담거나 개별 변수로 펼치지 말고 **type="rows"** 로 선언하라 — 행 수 제한이 사라지고 회사 문서의 표가 그대로 파싱된다.
...
- **규정 표(rows) 조회**(직급 판정, 직급별 하한·표준·상한 꺼내기 등)는 **rowcalc out="pick"** —
  필터로 해당 행을 특정하고(컬럼 vs 전역 변수/산출 비교), expression 에 꺼낼 컬럼명 하나:
  부여직급코드 = { "type":"rowcalc", "ref":"직급체계표", "out":"pick", "expression":"직급코드",
    "filters":[{"col":"최소경력","op":"<=","val":"최종경력연차"},{"col":"최대경력","op":">=","val":"최종경력연차"}] }
