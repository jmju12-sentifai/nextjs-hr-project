      ...
      >
        ...
        <div className="mx-auto grid max-w-3xl gap-5">
          {LISTED_PLANS.map((plan) => (
            <div
              key={plan.key}
              className="flex flex-col gap-8 rounded-2xl border border-blue-200 bg-white p-8 shadow-sm ring-2 ring-blue-100 sm:flex-row sm:items-center"
            >
              <div className="sm:w-[42%]">
                <span className="inline-block rounded-full bg-blue-50 px-3 py-1 text-[11px] font-bold tracking-wider text-blue-700">
                  {plan.badge}
                </span>
                ...
                <p className="mt-2 text-xs leading-relaxed text-gray-500">{plan.sub}</p>
                <Link
                  href={`/payment?plan=${plan.key}`}
                  className="mt-6 block w-full rounded-lg bg-blue-700 px-4 py-2.5 text-center text-sm font-medium text-white transition hover:bg-blue-800"
                >
                  구독 시작하기 →
                </Link>
              </div>
              <ul className="flex-1 space-y-2 border-t border-gray-100 pt-6 sm:border-l sm:border-t-0 sm:pl-8 sm:pt-0">
                {plan.features.map((f) => (
