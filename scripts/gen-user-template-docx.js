...
const cell = (text, widthDxa, { bold = false, fill } = {}) =>
  new TableCell({
    ...
    children: [new Paragraph({ spacing: { after: 0 }, children: [new TextRun({ text: String(text), font: FONT, bold, size: 16 })] })],
  });
...
const W_VARS = [1700, 1400, 800, 3700, 1600]; // 변수명·상위변수·필수·설명·예시
...
[
  ...
  "값이 정해진 항목(…유형·…방식·분기축 등)은 설명 칸에 「값: 후보1/후보2/…」 로 허용값을 전부 나열하세요 — 그 항목은 앱에서 선택형(콤보박스)이 되고, 문서 파싱도 그 값 중에서만 고릅니다.",
  "「값: …」 을 적지 않은 항목은 자유 입력(텍스트)으로 남습니다. 경력내역·보유자격처럼 여러 건을 적는 목록 항목에는 「값: …」 표기를 쓰지 마세요 — 목록은 선택형이 아닙니다.",
...
c.push(tbl([
  ...
  ["지급방식", "운영기준", "필수", "지원금 지급 방식. 값: 일시금/분할", "일시금"],
...
c.push(tbl([
  ...
  ["대상자관계", "—", "선택", "대상자와의 관계. 값: 본인/배우자/자녀", "본인"],
